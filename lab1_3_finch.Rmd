...
<ul class="pager">
  <li class="previous"><a href="lab1_2_calc.html">&larr; Part 2. R as a Calculator</a></li>
  <li class="next"><a href="lab1_4_bluestem.html">Part 4. Bluestem Data Assignment &rarr;</a></li>
</ul>
...
Download the data and save it in the project directory:

<a href="finches_data.xlsx"><i class="fas fa-file-excel fa-5x"></i></a>

If prompted for a location, save the file to your project directory. 

If not prompted, it probably saved the file to your Downloads folder. Open the Finder, click Downloads, drag the file into Documents, click Documents, and drag the file into your project directory.

<span class="label label-default">Important</span> Do not proceed until you see **finches_data.xlsx** in the Files tab (bottom right pane in RStudio).
...
1. Click the **New File** button on the toolbar (leftmost button with a green plus sign).
2. Click **R Script**
...
4. Give it a memorably name like **finches**. RStudio will automatically add the **.R** file extension.
...
You can delete `sqrt(9)` from your script now.

<p class="text-info"><span class="label label-info">Tip</span> From now on, run code from a script rather than the console.</p>

<p class="text-success"><span class="label label-success">Pro Tip</span> Copy code directly from the tutorial and paste it into your R script, comments and all. It saves time and you're less likely to make a typo.</p>
...
Before you can use a package, you to download and install it.

#### Install tidyverse
 
1. Click the **Package** tab in the botom right pane and 
2. Click **Install**. 
3. Type the name of the package, `tidyverse`
4. Click **Install**
5. If a question appears in the Console window asking if you want to install from binary, put your cursor in the console, type "n", and press enter.

You only have to install a package once on each computer you are using. If you open RStudio again later, every package you've ever installed will still be there.

#### Load the packages

<p class="text-info">Copy these two lines (including the comments) and paste them into your script.</p>

Do not copy the output below these two lines. That is just to show you what will be output in the console when you run the code.
...
<span class="label label-warning">Warning</span> If you haven't saved your data file in the right place, or if you are not working with your RStudio project open, or if you renamed your finch data file during download, then you will get an error message that says:

Error: `path` does not exist: ‘finches_data.xlsx’

To solve this error, make sure you have the data file in your project folder and the file name looks exactly like it does in the code, `finches_data.xlsx`.
...
Look in your project directory and you'll see a file named **Beak Length Histogram.png** now. You can click it to open the file in your computer's default image viewer.
...
```{r}
# summarize the dataset by survival outcome
grouped_summary <- 
  finches %>% 
  group_by(outcome) %>% 
  summarize(mean = mean(beak_length),
...
ggplot(
  data = grouped_summary,       # use this instead of finches dataset
  ...
  geom_errorbar(
    ...
    width = .3                   # width of horizontal part of error bars
  ) +
...
<blockquote class="text-info">Mean beak depths were 11.1 mm in survivors and 10.5 mm in non-survivors (Figure 1). This difference was stastistically significant according to a Welch's two-sample t-test assuming unequal variances (t=3.6335, df=94.807, p=0.0004539).</blockquote>
