...
```{r}
mean(x) + 2 * sem  # upper limit
mean(x) - 2 * sem  # lower limit
c(mean(x) + 2 * sem, mean(x) - 2 * sem)
```

### Self-Assessment

Image you collected a sample of data which consisted of the following values:
...
#### [Part 3. Finches dataset &rarr;](lab1_finch.html)